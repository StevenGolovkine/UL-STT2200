...
data = FileAttachment("./data_fit.csv").csv({ typed: true })
data_mse = FileAttachment("./data_mse.csv").csv({ typed: true })
...
points = data.filter(function(df) {
  return df.span == 0;
})
...
mse = data_mse.filter(function(df) {
  return df.component == 'MSE';
})
bias = data_mse.filter(function(df) {
  return df.component == 'Biais2';
})
variance = data_mse.filter(function(df) {
  return df.component == 'Variance';
})
